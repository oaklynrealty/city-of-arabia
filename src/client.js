(function () {
  ...
  function sanitizePhoneLocal(value) {
    return String(value || "").replace(/[^\d]/g, "").slice(0, 12);
  }

  function isValidPhoneNumber(localValue, countryCode) {
    const localDigits = sanitizePhoneLocal(localValue).replace(/^0+/, "");
    const countryDigits = String(countryCode || "").replace(/[^\d]/g, "");
    const totalDigits = countryDigits + localDigits;

    return localDigits.length >= 6 && localDigits.length <= 12 && totalDigits.length >= 8 && totalDigits.length <= 15;
  }
  ...
  Object.keys(fields).forEach(function (key) {
    ...
    field.input.addEventListener("input", function () {
      if (key === "phone") {
        const cleaned = sanitizePhoneLocal(field.input.value);
        if (field.input.value !== cleaned) field.input.value = cleaned;
      }
  ...
  form.addEventListener("submit", function (event) {
    ...
    Object.keys(fields).forEach(function (key) {
      ...
      const isValid =
        key === "phone"
          ? isValidPhoneNumber(inputValue, phoneCountryInput ? phoneCountryInput.value : "+971")
    ...
    const phoneLocal = sanitizePhoneLocal(fields.phone.input.value.trim());
